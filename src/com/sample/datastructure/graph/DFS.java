...
has n nodes. So instead of left and right traversal, we will traverse in loop.
...
public class DFS {
  ...
  public static void main(String[] args) throws Exception {
    ...
    int nodeCount = Integer.parseInt(nodeEdgeCount[0]);
    int edgeCount = Integer.parseInt(nodeEdgeCount[1]);
    Map<Integer, List<Integer>> adjMatrixMap = new HashMap<>();
    for (int i = 0; i < edgeCount; i++) {
      String[] nodeFromTo = s.nextLine().split(" ");
      int nodeFrom = Integer.parseInt(nodeFromTo[0]);
      int nodeTo = Integer.parseInt(nodeFromTo[1]);
      updateAdjMatrixMap(adjMatrixMap, nodeFrom, nodeTo);
      updateAdjMatrixMap(adjMatrixMap, nodeTo, nodeFrom);
    }
    int rootNode = Integer.parseInt(s.nextLine());
