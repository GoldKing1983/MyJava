...
public class RearrangeStringKDistanceApart2 {
  public String rearrangeString(String str, int k) {
    ...
    Queue<Map.Entry<Character, Integer>> q =
        new PriorityQueue<>((a, b) -> b.getValue() - a.getValue()); // PriorityQueue, order by freq
    ...
    while (!q.isEmpty()) { // q hold all currently available (ch, freq) pairs, all non-valid ones r in waitList
      ...
      waitList.add(entry);
      if (result.length() >= k) { // passed intial k chars, waitList full, start to release // passed in
        ...
        if (front.getValue() > 0) q.offer(front); // released ch might not be available any more (all freq used up)
      }
    }
    return (result.length() == str.length()) ? result.toString() : ""; // (ch, freq) stuck in waitList
  }
}
