...
public class WordLadderII_Recursion {
      ...
      for (char j = 'a'; j <= 'z'; j++) {
        ...
        if (!isVisited.contains(permutedString)) {
          if (wordListSet.contains(permutedString)) {
            ...
            isVisited.remove(
                permutedString); // remove previous visited nodes to continue with other
                                 // combinations
          }
        }
      }
  ...
  public static void main(String[] args) {
    ...
    System.out.println(
        w.findLadders("hit", "cog", Arrays.asList("hot", "dot", "dog", "lot", "log", "cog"))); //  5
    // System.out.println(w.findLadders("hit", "cog", Arrays.asList("hot", "dot", "dog", "lot",
    // "log"))); //  0
  }
}
