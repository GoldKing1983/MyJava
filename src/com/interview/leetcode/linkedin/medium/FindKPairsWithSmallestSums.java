...
public class FindKPairsWithSmallestSums {
  public List<int[]> kSmallestPairs(int[] nums1, int[] nums2, int k) {
    ...
    if (nums1 == null || nums1.length == 0 || nums2 == null || nums2.length == 0 || k <= 0)
      return res;
    for (int j = 0; j <= num2Size - 1; j++) pq.offer(new Tuple(0, j, nums1[0] + nums2[j]));
    // some time k may beyond the permutation sum. 
    // Ex: for 3,3 input k=10...Even though we can form only 9 result. For 10 th result we can skip the loop.  
    for (int i = 0; i < Math.min(k, num1Size * num2Size); i++) {
      System.out
          .println(pq.peek().sum + "[" + pq.peek().num1Index + " " + pq.peek().num2Index + "]");
      Tuple t = pq.poll();
      res.add(new int[] {nums1[t.num1Index], nums2[t.num2Index]});
      if (t.num1Index == num1Size - 1) continue;
      pq.offer(
          new Tuple(t.num1Index + 1, t.num2Index, nums1[t.num1Index + 1] + nums2[t.num2Index]));
    }
    return res;
  }
