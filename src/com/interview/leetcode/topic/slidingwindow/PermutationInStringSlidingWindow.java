...
1) Initialize freqMap with s1 "character" and its "-count".
		=====Initial Window======
2) Form the Initial Window. Slide s2 from 0 to s1.length. If a match found. Increment freqMap and increase matchCount.
...
5) From the left, elements are removed out of window. So decrement freqMap.
6) From the right, elements are added to window. So increment freqMap.
  ...
  public boolean checkInclusion(String s1, String s2) {
    ...
    while (right < s1.length()) {
      ...
      if (freqMap.containsKey(rightChar)) {
        int count = freqMap.get(rightChar) + 1;
        freqMap.put(rightChar, count);
    ...
    for (int left = 0; right < s2.length(); left++, right++) {
      ...
      if (freqMap.containsKey(leftChar)) {
        int leftCharCount = freqMap.get(leftChar);
        if (leftCharCount == 0) matchCount--;
        freqMap.put(leftChar, leftCharCount - 1); // removing from window so -1
      }
      if (freqMap.containsKey(rightChar)) {
        int rightCharCount = freqMap.get(rightChar) + 1; // adding to window so +1
        if (rightCharCount == 0) matchCount++;
        freqMap.put(rightChar, rightCharCount);
      }
      if (matchCount == freqMap.size()) return true;
    }
    return false;
