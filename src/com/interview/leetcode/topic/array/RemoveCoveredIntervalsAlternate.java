...
Output: 1
 */
public class RemoveCoveredIntervalsAlternate {
  public int removeCoveredIntervals(int[][] intervals) {
    Arrays.sort(intervals, (a, b) -> (a[0] == b[0] ? b[1] - a[1] : a[0] - b[0]));
    int resultCount = intervals.length; // Initially assume everything as result
    int previousIntervalEnd = intervals[0][1];
    for (int i = 1; i < intervals.length; i++) {
      int currentIntervalEnd = intervals[i][1];

      if (currentIntervalEnd <= previousIntervalEnd) {
        --resultCount; // Merge happend. So reduce resultCount
      } else {
